...
/**
 * A factory function that returns another functions whihc serves as
 * an event handler. When the function is invoked it created a event 
 * handler function. In order to determine which book the user is currently clicking on the
 ...
 * When the event handler is triggered by a click event, it uses the captured variables
 * to perform certain actions such as updating the preview details based
 * on the clicked book
...
const bookPreview = createBookPreviewHandler(books, authors, html)
html.list.item.addEventListener("click", bookPreview);
addBookPreviewHandler();
